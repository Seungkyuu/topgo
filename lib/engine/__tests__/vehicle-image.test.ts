...
describe("차량 이미지 매칭 (겟챠 모델 단위 사진)", () => {
  ...
  it("국산차도 모델 사진에 연결된다 (28개 브랜드 전체 시드)", () => {
    expect(imageForVehicle("현대", "그랜저 2.5")).toContain("img.getcha.io");
    expect(imageForVehicle("기아", "쏘렌토 하이브리드")).toContain("img.getcha.io");
  });
  ...
  it("인덱스의 BMW 차량 상당수가 이미지를 얻고, 얻은 건 모두 유효 URL이다", () => {
    ...
    for (const v of withImg) {
      expect(v.image).toMatch(/^https:\/\/img\.getcha\.io\//);
    }
  });
  ...
  it("'클래스' 같은 장식성 접미사 때문에 매칭이 통째로 실패하지 않는다 (벤츠)", () => {
    ...
    expect(imageForVehicle("벤츠", "E 300 4M AMG Line")).toContain("img.getcha.io");
    const index = buildVehicleIndex();
  ...
  it("마이바흐(영문 라벨)는 S-클래스가 아니라 마이바흐 사진으로 연결된다", () => {
    ...
    expect(maybach1).toContain("img.getcha.io");
